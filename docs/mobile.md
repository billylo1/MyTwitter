...
| Mac (Designed for iPhone) | Window locked to a phone-narrow width (~390pt) and display-tall height via `UIWindowScene.sizeRestrictions`. WKWebView pins to the view edges (not safe area) so no black gap appears under the Mac title bar |
