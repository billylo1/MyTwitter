...
| Cloud Functions | OAuth, sync, usage metering, `createInvite`, `getAuthorCard`, `setFollowing`, `setLiked` |
...
| `users/{xUserId}/posts/{tweetId}` | Read **own** feed | `text`, author fields, `media[]`, `linkPreview`, `url`, `isRetweet`, repost metadata, timestamps |
| `users/{xUserId}/authors/{id}` | Read **own** (rules) | Author profiles cached on sync; UI loads live cards via `getAuthorCard` |
...
| `config/public` | Read if member | `usage.*`, `lastRefreshedAt`, `invitesEnabled` (default `false`); legacy `defaultUid` / `defaultHandle` may exist from CLI oauth scripts |
...
- Video posts play inline (`<video controls>`); animated GIFs autoplay muted and loop. Card tap still opens X except on video controls / links / link previews.
...
- Header: title, handle, relative refresh time, info, sign out on one line. Info dialog: status, usage (cumulative + this cycle), admin invite button (only if `invitesEnabled`).
